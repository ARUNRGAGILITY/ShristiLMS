...
class QuizMaster {
  ...
  async downloadCertificate() {
    try {
      ...
      pdf.setDrawColor(251, 191, 36);
      pdf.setLineWidth(2);
      pdf.line(pageWidth / 2 - nameWidth / 2 - 10, currentY, pageWidth / 2 + nameWidth / 2 + 10, currentY);
      currentY += 15;
      ...
      pdf.setFontSize(11);
      pdf.setTextColor(37, 99, 235);
      pdf.text(`Score: ${score} | ${grade}`, pageWidth / 2, currentY + 12, { align: 'center' });
      currentY += 25;
      ...
      // Skill text - SMALL and positioned safely
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(8);
      pdf.setTextColor(100, 116, 139);
      pdf.text(`Demonstrating proficiency in ${this.config.skillArea}`, pageWidth / 2, currentY, { align: 'center' });
      
      // FOOTER - Fixed at bottom with ample space
      const footerY = pageHeight - 35;
      
      // Footer line
      pdf.setDrawColor(226, 232, 240);
      pdf.setLineWidth(1);
      pdf.line(margin + 20, footerY - 5, pageWidth - margin - 20, footerY - 5);
      ...
      // Left column
      pdf.text('EXAM DATE:', margin + 25, footerY);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(8);
      pdf.setTextColor(55, 65, 81);
      pdf.text(examDate, margin + 25, footerY + 5);
      
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(7);
      pdf.setTextColor(100, 116, 139);
      pdf.text('EXAM TIME:', margin + 25, footerY + 12);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(8);
      pdf.setTextColor(55, 65, 81);
      pdf.text(examTime, margin + 25, footerY + 17);
      
      // Center column
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(7);
      pdf.setTextColor(100, 116, 139);
      pdf.text('DURATION:', pageWidth / 2, footerY, { align: 'center' });
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(8);
      pdf.setTextColor(55, 65, 81);
      pdf.text(duration, pageWidth / 2, footerY + 5, { align: 'center' });
      
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(7);
      pdf.setTextColor(100, 116, 139);
      pdf.text('QUESTIONS:', pageWidth / 2, footerY + 12, { align: 'center' });
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(8);
      pdf.setTextColor(55, 65, 81);
      pdf.text(this.currentQuestions.length.toString(), pageWidth / 2, footerY + 17, { align: 'center' });
      
      // Right column
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(7);
      pdf.setTextColor(100, 116, 139);
      pdf.text('CERTIFIED BY:', pageWidth - margin - 25, footerY, { align: 'right' });
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(9);
      pdf.setTextColor(37, 99, 235);
      pdf.text(this.config.certifyingAuthority, pageWidth - margin - 25, footerY + 5, { align: 'right' });
      
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(6);
      pdf.setTextColor(100, 116, 139);
      pdf.text(this.config.platformName, pageWidth - margin - 25, footerY + 12, { align: 'right' });
      pdf.text(this.config.validationText, pageWidth - margin - 25, footerY + 17, { align: 'right' });
      
      // Certificate ID at very bottom
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(6);
      pdf.setTextColor(156, 163, 175);
      pdf.text(`Certificate ID: ${certificateId}`, pageWidth / 2, pageHeight - 8, { align: 'center' });
